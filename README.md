...
Документация по модулю Chunking: [`chunking/README.md`](chunking/README.md)
...
Модуль `chunking/` отвечает за загрузку документов и разбиение текста на чанки
перед построением embedding-векторов. Он поддерживает TXT, Markdown, HTML и PDF,
а также три стратегии разбиения:

- `fixed` - символьные окна фиксированного размера с overlap;
- `by_header` - разбиение Markdown-like документов по заголовкам;
- `semantic` - сборка чанков по абзацам и предложениям.
...
Результат `ingest()` - список `Chunk`, который дальше передается в
`embeddings/` для векторизации и в `vector_store/` для хранения. Подробнее:
...
Запускает демонстрацию загрузки документа и трех стратегий чанкинга без внешних
сервисов:

```bash
PYTHONPATH=. .venv/bin/python -m chunking.example
```

По умолчанию выводятся первые 10 чанков для каждой стратегии.
Текст чанка выводится как preview на 90 символов; полный текст можно показать
через `--preview-chars 0`.

С собственным документом:

```bash
PYTHONPATH=. .venv/bin/python -m chunking.example --source README.md --strategy by_header --chunk-size 500 --limit 10
```
